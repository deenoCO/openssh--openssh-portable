...
Match statements should have additional keywords representing all the "tokens" that would
be available to Match exec sub-statements.  This simplifies/streamlines ssh-config files
by avoiding using exec "..." to start a shell to do simple matching of tokens.

        keyword suggestion      token   value/meaning

        home                    %d      Local user's home directory.

        uid                     %i      The local user ID.
        ...
        port                    %p      The remote port.

DONE    localuser               %u      The local username.


        connhash                %C      Hash of %l%h%p%r.
        ...
        proxyjump               %j      The contents of the ProxyJump option,
                ...
                } else if (strcasecmp(attrib, "shortlocalhost") == 0) {
                } else if (strcasecmp(attrib, "localhost") == 0) {
                } else if (strcasecmp(attrib, "port") == 0) {
                } else if (strcasecmp(attrib, "connhash") == 0) {
                } else if (strcasecmp(attrib, "proxyjump") == 0) {


Currently avaliable keywords for tokens:

        originalhost            %n      The original remote hostname, as given on the command line.
        host                    %h      The remote hostname.
        user                    %r      The remote username.


Out of all the tokens, I would use %k (suggested as "hostkeyalias") and %L (suggested as
"shortlocalhost") the most.

I have some usages for %p (suggested as "port") and %u (suggested as "localuser") as well but
my main argument for keywords for the rest of the tokens available to exec is for 'completeness'
to support possible future use cases.
...
TOKENS
     Arguments to some keywords can make use of tokens, which are expanded at runtime:
           ...
           %I    A string describing the reason for a KnownHostsCommand execution: either ADDRESS
                 when looking up a host by address (only when CheckHostIP is enabled), HOSTNAME
                 when searching by hostname, or ORDER when preparing the host key algorithm
                 preference list to use for the destination host.
           %i    The local user ID.
           %j    The contents of the ProxyJump option, or the empty string if this option is
                 unset.
     ...
     CertificateFile, ControlPath, IdentityAgent, IdentityFile, Include, KnownHostsCommand,
     LocalForward, Match exec, RemoteCommand, RemoteForward, RevokedHostKeys, and
     UserKnownHostsFile accept the tokens %%, %C, %d, %h, %i, %j, %k, %L, %l, %n, %p, %r, and %u.
     ...
     ssh(1) performs no filtering or escaping of characters that have special meaning in shell
     commands (e.g. quotes), it is the user's responsibility to ensure that the arguments passed
     to ssh(1) do not contain such characters and that tokens are appropriately quoted when used.
--------------------------------------------------------------------------------------------------
#endif
